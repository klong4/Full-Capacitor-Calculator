...
from utils.calculations import (
    calculate_total_capacitance_parallel,  # Make sure to have this function in your utils
    calculate_energy_storage,
    calculate_energy_density
)

class ParallelCircuitsTab(QtWidgets.QWidget):
    def __init__(self):
        super().__init__()
    ...
    def calculate(self):
        try:
            ...
            volume = float(self.volumeInput.text())
            ...
            total_capacitance = calculate_total_capacitance_parallel([capacitance] * num_capacitors)  # F
            ...
            circuit_drawing = "|| " * num_capacitors

            results = (
                f"Number of Capacitors in Parallel: {num_capacitors}\n"
                f"Total Capacitance in Parallel (F): {total_capacitance}\n"
                f"Total Energy in Parallel (J): {total_energy_joules}\n"
                f"Energy Density (Wh/kg): {energy_density_input}\n"
                f"Nominal Voltage (V): {nominal_voltage}\n" 
                f"Circuit Diagram:\n{circuit_drawing}"
            )
            ...
            # Calculations
            calculations = (
                f"Total Capacitance in Parallel: {capacitance} F * {num_capacitors} = {total_capacitance} F\n"
                f"Total Energy in Parallel: 0.5 * {total_capacitance} F * ({nominal_voltage} V)^2 = {total_energy_joules} J"
            )
